...
namespace XamProjectTemplate
{
        ...
        public object ProvideValue(IServiceProvider serviceProvider)
        {
            ...
            if (provideValueTarget != null)
            {
                ...
                if (!string.IsNullOrEmpty(Value))
                {
                    ...
                    var returnData = Convert.ChangeType(IsResponsive ? numberValue.ScaleHeightResponsive() : numberValue.ScaleHeight(), Type.GetType(property.ToString()));
                    App.Log($"SCALING_ENDING : IsResponsive? => {IsResponsive} | Value => {returnData} :TYPE {returnData.GetType()} | {Type.GetType(property.ToString())}", this.GetType().ToString());
                    if (returnData != null)
                        return returnData;
    ...
    public class ScaleRowDefinition : IMarkupExtension
    {
        ...
        public object ProvideValue(IServiceProvider serviceProvider)
        {
            ...
            throw new InvalidOperationException($"Cannot convert RowDefination[{Value}]");
        }
    }
    ...
    public class ScalePoint : IMarkupExtension
    {
        public string Value { get; set; }
        public bool IsResponsive { get; set; }
        public ScaleOrientation ScaleType { get; set; } = ScaleOrientation.Balance;

        static PointTypeConverter pointTypeConverter = new PointTypeConverter();
        public object ProvideValue(IServiceProvider serviceProvider)
        {
            App.Log($"SCALING_STARTING : IsResponsive? => {IsResponsive} | Value => {Value}", this.GetType().ToString());
            if (!string.IsNullOrEmpty(Value))
            {
                ...
                Point point = (Point)pointTypeConverter.ConvertFromInvariantString(Value);
                switch (ScaleType)
                {
                    case ScaleOrientation.Balance: return IsResponsive ? new Point(point.X.ScaleWidthResponsive(), point.Y.ScaleHeightResponsive()) : new Point(point.X.ScaleWidth(), point.Y.ScaleHeight());
                    case ScaleOrientation.Width: return IsResponsive ? new Point(point.X.ScaleWidthResponsive(), point.Y.ScaleWidthResponsive()) : new Point(point.X.ScaleWidth(), point.Y.ScaleWidth());
                    case ScaleOrientation.Height: return IsResponsive ? new Point(point.X.ScaleHeightResponsive(), point.Y.ScaleHeightResponsive()) : new Point(point.X.ScaleHeight(), point.Y.ScaleHeight());
                }
                return new Point(point.X.ScaleWidth(), point.Y.ScaleHeight());
            }
