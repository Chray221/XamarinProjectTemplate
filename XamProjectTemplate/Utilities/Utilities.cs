...
namespace XamProjectTemplate
{
    ...
    { 
        public static string InsertEpoch(string payload)
        {
            ...
            //return payload.Insert(payload.Length - 1, ",\"e\":" + DateTime.Now/*+ TimeManager.Instance.GetDateTime(DateTimeOffset.UtcNow.ToUnixTimeMilliseconds())*/);
            return payload;
        }

        public static string InsertEpochUrl(string url)
        {
            //return url + (url.Contains("?") ? "&" : "?") + "e=" + DateTime.Now/*+ TimeManager.Instance.GetDateTime(DateTimeOffset.UtcNow.ToUnixTimeMilliseconds())*/;
            return url;
        }
